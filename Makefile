...
sh: .iid/sh .PHONY
	$(DOCKER) run --rm -v $(PWD)/bench:/bench -v $(PWD):/work -w /work/bin -it javascripten-sh
...
bin/$(1): .iid/$(1)
	@mkdir -p bin
	@rm -f "bin/$(1)"
	@$(DOCKER) run --rm -v $(PWD)/bin:/work/bin -it javascripten-$(1) /bin/bash -c ' \
	  if [[ -f "$$$$JS_BINARY" ]]; then \
	    strip "$$$$JS_BINARY" 2>/dev/null && cp -fv "$$$$JS_BINARY" "/work/bin/$(1)"; \
	  elif [[ -f "$$$$JS_BINARY_LINK" ]]; then \
	    ln -sv "$$$$JS_BINARY_LINK" "/work/bin/$(1)"; \
	  fi' || true
...
metadata: .PHONY
	...
	@for e in $(ENGINES); do \
	  ...
	  rev=`$(DOCKER) run --rm javascripten-$$e /bin/bash -c 2>/dev/null \
	       "git rev-parse HEAD"`; \
	  size=`[[ $$(file -b --mime-type bin/$$e 2>/dev/null) == */*-executable && "$$e" != graaljs ]] && \
